    ...
    <div class="col-md-6">
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <br />
                Ostatnie siku:
                <?= $lastPee['timestamp'] ?><br />
                <span class="<?= $lastPee['warning'] == 1 ? 'text-danger' : 'text-info' ?>"> (<?= showDecimal($lastPee['days'], 1) ?> dni temu)</span>
                <br />
                Zapotrzebowanie dzisiaj:
                <?= showInt($dailyDemand['total']); ?> %<br />
                Zapotrzebowanie wczoraj:
                ...
                <a class="btn btn-primary"
                    href="<?= baseUrl() ?>/src/CatFeeding/Views/Reports/timeline.php?CatId=<?= $catId ?>">Oś
                    czasu</a>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-6">
        <div class="card mb-3">
            ...
            <div class="card-body">
                <?php
                if ($medicineDoses)
                    foreach ($medicineDoses as $dose) {
                        ?>
                        <div class="mb-3">Podano
                            <?= $dose['medicine_name'] ?>
                            <?= showDecimal($dose['applied'], 4) ?>
                            z
                            <?= showDecimal($dose['expected'], 4) ?>
                            <?= $dose['unit'] ?>
                        </div>
                        <?php
                        if ($dose['expected'] > $dose['applied']) {
                            ...
                            <form action="../UseCases/ApplyMedicineUseCase.php" method="post">
                                ...
                                <button class="btn btn-primary mb-3">Podaj
                                    <?= $dose['name'] ?>
                                </button>
                            </form>
                            <?php
                        ...
                        if ($dose['applied'] > 0) {
                            ...
                            <form action="../UseCases/UndoMedicineApplicationUseCase.php" method="post">
                                ...
                                <button class="btn btn-success mb-3">Cofnij
                                    <?= $dose['name'] ?>
                                </button>
                            </form>
                            <?php
...
<div class="card mb-3">
    ...
    <div class="list-group list-group-flush">
        ...
        foreach ($lastMeals as $lastMeal) {
            ...
            <a href="<?= './meal.php?Id=' . $lastMeal['id'] ?>" class="list-group-item list-group-item-action">
                ...
                <p class="mb-1">
                    <?= $lastMeal['start_weight'] ?> -
                    <?= $lastMeal['end_weight'] ?>
                    =
                    <?= showInt($lastMeal['start_weight'] - $lastMeal['end_weight']) ?> g
                    =
                    <?= showInt($lastMeal['daily_demand_percentage']) ?> %
                </p>
                <small>
                    <?= $lastMeal['start'] ?> -
                    <?= $lastMeal['end'] ?>
...
<div class="card mb-3">
    ...
    <div class="card-body">
        <form action="../UseCases/WeighCatUseCase.php" method="post">
            <input type="hidden" name="CatId" value="<?= $catId ?>">
            <div class="form-group">
                <label for="CatWeight">Waga</label>
                <input id="CatWeight" type="number" name="Weight" class="form-control" required min="2" step="0.01"
                    max="5"></textarea>
            </div>
            <div class="form-group">
                <button type="submit" class="btn btn-primary">Dodaj</button>
